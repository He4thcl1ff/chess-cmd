...
class pawn(piece):
    ...
    def check_available(self):
        ...
        if self.colour == 'W':
            ...
            direction = -1
        for i in range(-1, 2):
            if not self.board.check_border([chr(ord(self.position[0]) + direction), self.position[1] + i]):
                c = [chr(ord(self.position[0]) + direction), self.position[1] + i]
                if self.board.piece(c).name == '-' and i == 0:
                    self.available += [c]
                    if self.colour == 'W' and self.position[0] == 'B' and self.board.piece(
                            [chr(ord(c[0]) + 1), c[1]]).name == '-':
                        self.available += [[chr(ord(c[0]) + 1), c[1]]]
                    elif self.colour == 'B' and self.position[0] == chr(
                            65 + (self.board.n - 1) - 1) and self.board.piece(
                            [chr(ord(c[0]) - 1), c[1]]).name == '-':
                        self.available += [[chr(ord(c[0]) - 1), c[1]]]
